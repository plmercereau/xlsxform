...
const _QUESTION_TYPE_DICT: Record<string, QuestionTypeEntry> = {
	"q picture": {
...
export const QUESTION_TYPE_DICT: Readonly<Record<string, QuestionTypeEntry>> =
...
export function getMetaGroup(
	...
	if (!omitInstanceID) {
		metaChildren.push({
			...
			[constants.BIND]: {
				readonly: "true()",
				"jr:preload": instanceId,
			},
		});
	}

	metaChildren.push(...children);

	return {
		[constants.NAME]: "meta",
		[constants.TYPE]: constants.GROUP,
		[constants.CONTROL]: { bodyless: true },
		[constants.CHILDREN]: metaChildren,
	};
}
