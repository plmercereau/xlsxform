...
function _xpq_model_itext_label(q_name: string, lang: string, q_label: string): string {
...
function xpq_body_upload_tags(qname: string, tags: [string, string][]): string {
	const tags_xp = tags.map(([k, v]) => `./x:tag[@key='${k}']/x:label[text()='${v}']`).join("\n          and ");
...
function xpc_model_instance_choices_label(cname: string, choices: [string, string][]): string {
	const choices_xp = choices.map(([cv, cl]) =>
	...
	it("test_osm_type_with_select", () => {
		assertPyxformXform({
			...
			xml__xpath_match: [
				...
				xpc_model_instance_choices_label("c1", [["n1", "l1"], ["n2", "l2"]]),
				...
				xpq_body_upload_tags("osm_build", [["name", "Name"], ["addr:city", "City"]]),
			],
	...
	it("test_osm_type_with_multiple_lists__separate", () => {
		assertPyxformXform({
			...
			xml__xpath_match: [
				...
				xpq_body_upload_tags("osm_room", [["room:type", "Type"], ["habitable", "Habitable"]]),
			],
	...
	it("test_osm_type_with_multiple_lists__shared", () => {
		assertPyxformXform({
			...
			xml__xpath_match: [
				...
				xpq_body_upload_tags("osm_resi", [["room:type", "Type"], ["habitable", "Habitable"]]),
				...
				xpq_body_upload_tags("osm_office", [["room:type", "Type"], ["habitable", "Habitable"]]),
			],
			xml__excludes: ["room_tags"],
		});
	});
